...
async function startTracking(device) {
  try {
    ...
    const response = await fetch(`http://18.209.6.96/devices/status/${device.imei}`);
    if (!response.ok) {
...
const cargarGeozonas = async () => {
  try {
    const response = await fetch('http://18.209.6.96/geozone/geozones');
    if (!response.ok) {
      throw new Error('Error en la respuesta de la API');
    }
    const data = await response.json();
...
const saveGeozone = async () => {
  ...
  try {
    const response = await axios.post('http://18.209.6.96/geozone/geozones', geozoneData);
...
const confirmCreateGeozona = async () => {
  ...
  try {
    ...
    const response = await axios.put(`http://18.209.6.96/devices/geozones/${selectedGeozone.value._id}`, geozoneData);
